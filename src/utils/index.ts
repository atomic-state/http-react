...
export function revalidateKey(key: any) {
  revalidate([key])
}
...
export function fetchOptions<T>(
  init: string | FetchConfigType<T>,
  options?: Omit<FetchConfigTypeNoUrl<T>, 'url'>
) {
  return typeof init === 'string'
    ? { url: init, ...options }
    : (init as FetchConfigType<T>)
}
